...
class BoxConfig(object):
    ...
    """
    filepath = os.path.expanduser('~/.noiselabs/box/box-sync.cfg')
    options = {'main': ['sync_dir', 'webdav']}
    ...
    def check_file(self):
        ...
        if not os.path.isfile(self.filepath):
            ...
            if not os.path.isdir(basedir):
                os.makedirs(os.path.dirname(self.filepath), 0700)
            # create the file
            f = open(self.filepath, 'wb')
            f.write("; Generated by %s-%s in %s" % (__prog__, __version__, str(datetime.date.today())+"\n"))
            f.close()
            print 'Created configuration file "%s".' % self.filepath
